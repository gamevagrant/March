...
public class DailyLandingActivities : MonoBehaviour
{
    ...
    private int m_showDay = 0;
    private string[] m_Items = null;
    private string[] m_Golds = null;
	...
	void Awake()
	{
        ...
        if (null != setting)
	    {
            ...
            m_showDay = Int32.Parse(setting["showDay"]);
            m_Golds = setting["gold"].Split('|');
            m_Items = setting["item"].Split('|');
            m_star = setting["star"].Split('|');
	...
	private void addItem(int i, string item) {
		...
		if (itemNum > 1) {
			...
			numText.GetComponent<Text> ().text = "×" + itemNum.ToString ();
		}
	}
	...
	IEnumerator dismissDay()
	{
		...
		i_go.transform.DOLocalMove (new Vector3 (0, -50, 0), 1f).OnComplete (() => 
			m_continueBtn.gameObject.SetActive (true)
		);
		i_go.transform.GetComponent<Image> ().DOFade (0, 1f).OnComplete (() => 
			sele_go.gameObject.SetActive (true)
		);
	}

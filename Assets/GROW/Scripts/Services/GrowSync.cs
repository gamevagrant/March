...
namespace Grow.Sync
{
	...
	public class GrowSync {
		...
		public static void Initialize(bool modelSync, bool stateSync) {
			ServicesEvents.Initialize();
			GrowUtils.LogDebug (TAG, "SOOMLA/UNITY Initializing Sync");
				...
				try {
					...
					if (storeInfoType != null) {
						...
						if (initFromDBMethod != null) {
							...
							GrowUtils.LogDebug(TAG, "Finished syncing model Unity");
						}
					}
				} catch (Exception ex) {
					...
					GrowUtils.LogError(TAG, message);
				...
				try {
					...
					if (storeInventoryType != null) {
						...
						if (refreshLocalInventoryMethod != null) {
							...
							GrowUtils.LogDebug(TAG, "Finished syncing state Unity");
						}
					}
				} catch (Exception ex) {
					// This should not happen since if native succeeded in loading, Unity should too
				...
				try {
					...
					if (soomlaProfileType != null) {
						...
						if (logoutFromAllProvidersMethod != null) {
							GrowUtils.LogDebug(TAG, "Logging out from all providers");
		...
		private const string TAG = "GROW Sync";
	}
}
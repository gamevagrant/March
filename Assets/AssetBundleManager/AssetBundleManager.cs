...
namespace AssetBundles
{
    ...
    public class AssetBundleManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Starts a load operation for all assets from the given asset bundle.
        /// </summary>
        static public AssetBundleLoadAllAssetOperation LoadAllAssetAsync(string assetBundleName, System.Type type)
        {
            Log(LogType.Info, "Loading from " + assetBundleName + " bundle");

            AssetBundleLoadAllAssetOperation operation = null;
#if UNITY_EDITOR
            if (SimulateAssetBundleInEditor)
            {
                ...
                if (assetPaths.Length == 0)
                {
                    Log(LogType.Error, "There is no asset with in " + assetBundleName);
                    return null;
                }

                // @TODO: Now we only get the main object from the first asset. Should consider type also.
                UnityEngine.Object[] target = assetPaths.Select(assetPath => AssetDatabase.LoadMainAssetAtPath(assetPath)).ToArray();
                operation = new AssetBundleLoadAllAssetOperationSimulation(target);
            }
            else
#endif
            {

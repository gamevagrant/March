...
namespace UnityQuickSheet
{
    ...
    public class ExcelMachineEditor : BaseMachineEditor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                var runtimePath = Application.dataPath + Path.DirectorySeparatorChar + machine.RuntimeClassPath;
                var editorPath = Application.dataPath + Path.DirectorySeparatorChar + machine.EditorClassPath;
                var pathList = new List<string> { runtimePath, editorPath };
                pathList.ForEach(assetPath =>
                    new DirectoryInfo(assetPath).GetFiles("*").ToList().ForEach(file => File.Delete(file.FullName)));
